...
public function getMatchingProperties($lead) {
    ...
    if (!empty($lead->propertyType)) {
        $this->db->where('p.property_type', $lead->propertyType);
    }
    
    // Sub property type matching
    if (!empty($lead->propertyType_sub)) {
        $this->db->where('p.type', $lead->propertyType_sub);
    }
    
    // Budget matching (show properties within lead's budget range)
    if (!empty($lead->max_budget) && is_numeric($lead->max_budget)) {
        $this->db->where('CAST(p.budget as DECIMAL) <=', $lead->max_budget);
    } elseif (!empty($lead->budget) && is_numeric($lead->budget)) {
        $this->db->where('CAST(p.budget as DECIMAL) <=', $lead->budget);
    }
    
    // Location matching
    if (!empty($lead->preferred_location)) {
        $this->db->group_start();
        $this->db->like('p.address', $lead->preferred_location);
        $this->db->or_like('p.city', $lead->preferred_location);
        $this->db->or_like('p.name', $lead->preferred_location);
        $this->db->group_end();
    } elseif (!empty($lead->city)) {
        $this->db->group_start();
        $this->db->like('p.address', $lead->city);
        $this->db->or_like('p.city', $lead->city);
        $this->db->or_like('p.name', $lead->city);
        $this->db->group_end();
    }
    
    // Property for matching (sale/rent)
    if (!empty($lead->leads_type)) {
        if (strtolower($lead->leads_type) == 'buyer') {
            $this->db->where('p.property_for', 'sale');
        } elseif (strtolower($lead->leads_type) == 'tenant') {
            $this->db->where('p.property_for', 'rent');
        }
    }
    
    $this->db->order_by('p.id', 'DESC');
    $this->db->limit(20); // Limit to 20 matching properties
